...
do
	...
	do
   		...
   		awk '(NR ==1) || (FNR > 1)' $state/$year/split_* > $state/$year/$year_$state_all.csv
   		zip $state/$year/$year_$state_all.zip $state/$year/$year_$state_all.csv

   		# move to data dir in repo
   		mv $state/$year/$year_$state_all.zip ../hmda/data/$state/$year
	done
done
